...
class OnlineChatHistoryScreen extends StatefulWidget {
  ...
  final List<dynamic> allClients;
  OnlineChatHistoryScreen({Key key, @required this.userInfo, this.clientInfo, this.allClients})
      : super(key: key);
  @override
  _OnlineChatHistoryScreenState createState() =>
      _OnlineChatHistoryScreenState();
}
...
class _OnlineChatHistoryScreenState extends State<OnlineChatHistoryScreen> {
  ...
  List<DropdownMenuItem> ownAgents = [];
  String transAgent;
  @override
  void initState() {
    super.initState();
    ...
    var curDatePrevTime = new DateTime.now();
        ...
        if (mounted) {
          ...
          setState(() {
            if ((widget.clientInfo["id"] == oneMsg["sender_id"] &&
              ...
              if(oneMsg.containsKey('line')){
                oneMsg["created_at"] = '';
                oneMsg['attach_file'] = '';
                oneMsg['attach_real_name'] = '';
                oneMsg['message'] = '';
                oneMsg['line'] = true;
            ...
            }else if(oneMsg["receiver_id"] == widget.userInfo.id && !oneMsg.containsKey('line')){
              widget.allClients.forEach((element) { 
                
                if(element["id"] == oneMsg["sender_id"] && localTime(oneMsg["created_at"]).compareTo(curDatePrevTime) > 0){
                  showOnlineChatAnother(element["name"] + " said : " + oneMsg["message"]);
                }
              });
    ...
    ipListener = dbRef
      ...
      if (mounted) {
        setState(() {
          ipaddress = event.snapshot.value != null ? event.snapshot.value : '';
        });
      }
    });
    ipstateListener = dbRef
        .child("ONLINECHAT")
        .child(widget.userInfo.id)
        .child('online')
        .child(widget.clientInfo["id"])
  ...
  void getAgents() {
    final body = {
      ...
      "exist_email": widget.clientInfo['email']
    };
    ApiService.getAgents(body).then((response) {
      setState(() {
        ...
        for (int i = 0; i < allAgents.length; i++) {
          ownAgents.add(new DropdownMenuItem(
            child: new Text(allAgents[i]['username']),
            value: allAgents[i]['id'],
          ));
        }
  ...
  void dispose() {
    ...
    if (ipstateListener != null) {
      ipstateListener.cancel();
    }
  ...
  Future<void> transferAgent() async {
    if (transAgent == null) {
      showErrorToast("Please select right agent.");
    } else {
      String title = 'Confirmation!';
      String content = 'Confirm transfer.';
      final ConfirmAction action = await confirmDialog(context, title, content);
      var curDateTime = new DateTime.now();
      var messageJSON = {
        "sender_id": widget.userInfo.id,
        "receiver_id": widget.clientInfo["id"],
        "message": "Click to connect with right agent.",
        "read_status": "NO",
        "created_at": curDateTime.toUtc().toIso8601String(),
        "zoneOffest": -curDateTime.timeZoneOffset.inMinutes,
        "attach_file": "",
        "attach_real_name": "",
    ...
    final writingRow = Row(
      children: <Widget>[
        Padding(
          ...
          child: Text(
            widget.clientInfo["name"] + " is typing",
      ...
      appBar: AppBar(
        ...
        children: <Widget>[
          ...
          Switch(
            ...
            inactiveThumbColor: ipstatus ? Colors.blue : Colors.white70,
            inactiveTrackColor:
                ipstatus ? Colors.grey.shade400 : Colors.grey.shade400,
              ...
              Container(
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    Flag(widget.clientInfo["country_code"], height: 15, width: 15,),
                  ],
                ),
              ),
              Card(
                  ...
                  subtitle: Padding(
                      ...
                      child: Row(
                        children: <Widget>[
                          ...
                          Text(msgCount.toString() + " Messages"),
                          Padding(
                            padding: EdgeInsets.fromLTRB(20.0, 0.0, 20.0, 0.0),
                            child: DropdownButton(
                              items: ownAgents,
                              value: transAgent,
                              hint: Text("Agents Name"),
                              onChanged: (value) {
                                setState(() {
                                  transAgent = value;
                                });
